import asyncio
import time
from typing import Optional

import aiohttp
import discord
from discord.ext import tasks
from config import (
    FIP_STREAMS,
    guild_station_map,
    live_messages,
    guild_song_ids,
    station_last_song_ids,
    normalize_pikapi_url,
)
from app.embeds.metadata_embed import fetch_metadata_embed, build_all_stations_embed
from app.ui.views import FIPControlView
from app.services.spotify import fetch_spotify_url
from app.db.session_store import get_station_now_playing, update_now_playing

bot = None
metadata_session: Optional[aiohttp.ClientSession] = None


async def get_session() -> aiohttp.ClientSession:
    global metadata_session
    if metadata_session is None or metadata_session.closed:
        metadata_session = aiohttp.ClientSession(
            timeout=aiohttp.ClientTimeout(total=10),
            connector=aiohttp.TCPConnector(limit=20, ttl_dns_cache=300),
        )
    return metadata_session


def set_song_updater_bot(bot_instance):
    global bot
    bot = bot_instance


async def build_payload(guild_id, genre):
    """Build (metadata_embed, summary_embed, view) used by both edit and resend paths."""
    title, artist = ("", "")
    row = await asyncio.to_thread(get_station_now_playing, genre)
    if row:
        _, full_title, *_ = row
        if " – " in full_title:
            title, artist = full_title.split(" – ", 1)

    spotify_url = await fetch_spotify_url(title, artist)
    metadata_embed = await fetch_metadata_embed(guild_id)
    summary_embed = build_all_stations_embed()
    view = FIPControlView(guild_id=guild_id, spotify_url=spotify_url)
    return metadata_embed, summary_embed, view


async def edit_live_message(guild_id, message, genre) -> bool:
    """Edit the existing now-playing message in place. Returns False if caller should resend."""
    metadata_embed, summary_embed, view = await build_payload(guild_id, genre)
    if not metadata_embed:
        return False
    try:
        await message.edit(embeds=[summary_embed, metadata_embed], view=view)
        return True
    except discord.NotFound:
        return False
    except discord.Forbidden as e:
        # Can't edit our own message anymore (channel perms changed). Stop tracking it.
        print(f"[Edit] Lost write access for guild {guild_id}: {e}. Dropping live message.")
        live_messages.pop(guild_id, None)
        return False
    except discord.HTTPException as e:
        print(f"[Edit] HTTP error for guild {guild_id}: {e}")
        return False


async def resend_message(guild_id, channel, genre, song_id):
    if not bot or not channel:
        print(f"[Resend] Skipped resend for guild {guild_id} — missing bot or channel.")
        return

    metadata_embed, summary_embed, view = await build_payload(guild_id, genre)
    if not metadata_embed:
        print(f"[Resend] Skipped resend — no metadata for guild {guild_id}")
        return

    try:
        new_message = await channel.send(
            content=f"🎶 Now playing FIP {genre} in {channel.name}",
            embeds=[summary_embed, metadata_embed],
            view=view,
        )
    except discord.Forbidden as e:
        # No Send Messages in this channel; drop the guild so the loop stops retrying.
        print(
            f"[Resend] Missing Send Messages in guild {guild_id} channel "
            f"{getattr(channel, 'id', '?')}: {e}. Dropping live message."
        )
        live_messages.pop(guild_id, None)
        return
    except discord.HTTPException as e:
        print(f"[Resend] HTTP error for guild {guild_id}: {e}")
        return

    live_messages[guild_id] = {"message": new_message, "channel": channel}
    guild_song_ids[guild_id] = song_id
    print(f"[Resend] Sent new message for guild {guild_id}")

@tasks.loop(seconds=2)
async def update_station_cache():
    now = int(time.time())
    session = await get_session()
    for genre, stream in FIP_STREAMS.items():
        try:
            row = await asyncio.to_thread(get_station_now_playing, genre)
            if row:
                _, _, _, end_time, _ = row
                if end_time and now < end_time:
                    continue

            url = f"https://fip-metadata.fly.dev/api/metadata/{stream['metadata']}"
            async with session.get(url) as resp:
                if resp.status != 200:
                    continue

                data = await resp.json()
                now_block = data.get("now")
                if not now_block:
                    continue

                # songUuid is the new field; older payloads nested it under song.id.
                song_id = (
                    now_block.get("songUuid")
                    or (now_block.get("song") or {}).get("id")
                )
                start = now_block.get("startTime")
                end = now_block.get("endTime")
                if start is None or end is None:
                    continue

                first_line = (now_block.get("firstLine") or {}).get("title", "") or ""
                second_line = (now_block.get("secondLine") or {}).get("title", "") or ""
                full_title = f"{first_line} – {second_line}"
                visuals = now_block.get("visuals") or {}
                thumbnail_url = normalize_pikapi_url(
                    (visuals.get("card") or {}).get("src")
                    or (visuals.get("player") or {}).get("src")
                )

                await asyncio.to_thread(
                    update_now_playing, genre, song_id, full_title, start, end, thumbnail_url
                )
                prev_song_id = station_last_song_ids.get(genre)
                station_last_song_ids[genre] = song_id

                if song_id and song_id != prev_song_id:
                    print(f"[Station Update] New song for {genre} → refreshing")
                    await refresh_simple_embeds()

        except Exception as e:
            print(f"[Cache Error] {genre}: {e}")

@tasks.loop(seconds=2)
async def update_song_embeds():
    now = int(time.time())

    for guild_id, data in list(live_messages.items()):
        message = data.get("message")
        channel = data.get("channel")
        genre = guild_station_map.get(guild_id)

        row = await asyncio.to_thread(get_station_now_playing, genre)
        if not row:
            continue

        song_id, _, start_time, _, _ = row
        prev_song_id = guild_song_ids.get(guild_id)

        is_new_song = song_id and song_id != prev_song_id
        is_old_message = start_time and (now - start_time >= 1800)  # 30 minutes

        if not (is_new_song or is_old_message):
            continue

        # On song change, edit in place. Only delete+resend on the 30 min bump, or if edit fails.
        if is_new_song and not is_old_message and message is not None:
            if await edit_live_message(guild_id, message, genre):
                guild_song_ids[guild_id] = song_id
                continue

        if message is not None:
            try:
                await message.delete()
            except Exception:
                pass
        live_messages.pop(guild_id, None)
        await resend_message(guild_id, channel, genre, song_id)

@tasks.loop(minutes=10)
async def refresh_simple_embeds():
    summary_embed = build_all_stations_embed()
    for guild_id, data in list(live_messages.items()):
        message = data.get("message")
        row = await asyncio.to_thread(
            get_station_now_playing, guild_station_map.get(guild_id)
        )
        if not row:
            continue

        metadata_embed = await fetch_metadata_embed(guild_id)
        if not metadata_embed:
            continue

        try:
            await message.edit(embeds=[summary_embed, metadata_embed])
        except Exception as e:
            print(f"[Refresh Embed Error] Guild {guild_id}: {e}")
